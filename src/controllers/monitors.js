...
async function getMonitors(req, res) {
    try {
        const monitors = await pool.query('SELECT id, title, url, is_active, created_at FROM monitors WHERE user_id = $1', [req.user.userId]);
...
async function getMonitorById(req, res) {
    try {
        const monitors = await pool.query('SELECT id, title, url, is_active, created_at FROM monitors WHERE user_id = $1 AND id = $2', [req.user.userId, req.params.id]);
...
async function createMonitor(req, res) {
    try {
        ...
        await pool.query('INSERT INTO monitors (user_id, title, url) VALUES ($1, $2, $3)', [req.user.userId, title, url]);
...
async function deleteMonitor(req, res) {
    try {
        const result = await pool.query('DELETE FROM monitors WHERE id = $1 AND user_id = $2', [req.params.id, req.user.userId]);
